...
\- \*\*Backend\*\*: `POST /api/student/request-bulk` validates stock and max-quantity settings per line item, then creates one `component\_requests` row per item (same optional note on each). Single-item flow remains available via `POST /api/student/request` if needed.
...
\- `id`, `name`, `data` (Full JSON snapshot), `created\_at`.
